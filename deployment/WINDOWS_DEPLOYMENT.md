# SGDI - Windows Server Deployment Guide

This guide provides instructions for deploying SGDI on Windows Server.
...
C:\nssm\nssm.exe set SistemaGED DisplayName "SGDI Application"
...
<configuration>
    <system.webServer>
        <rewrite>
            <rules>
                <rule name="SGDI Reverse Proxy" stopProcessing="true">
                    <match url="(.*)" />
...
```powershell
# Allow HTTP
New-NetFirewallRule -DisplayName "SGDI HTTP" -Direction Inbound -Protocol TCP -LocalPort 80 -Action Allow

# Allow HTTPS
New-NetFirewallRule -DisplayName "SGDI HTTPS" -Direction Inbound -Protocol TCP -LocalPort 443 -Action Allow
```
